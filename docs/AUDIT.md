...
| scatter-gather | ✅ | 新規執筆(2026-08-02)。新しい編「サービスのつなぎ方」の1本目。1台あたり5%の裾が、100台に配ると99.41%になることを測定。悪化が台数に比例せず数台で天井に張り付くこと、打ち切りは答えを払い2本目は下流の仕事を払うこと、2本目を投げ始める時刻を速い応答より手前に置くと全台が2本になること(投げた本数105→200)を固定。カバレッジ100% |
| deadline | ✅ | 新規執筆(2026-08-03)。同編2本目。3段・各段3回で末端が9回、4段で27回、5段で81回になることを測定(倍率は試行回数そのもの)。締め切りを段ごとに持つと入口の45を超えて60まで延び、うち10が誰も待っていない仕事になること、下へ渡すと段数によらず4回・経過40で頭打ちになることを固定。④で予算40だと5回目で通るものを落とすことも。カバレッジ100% |
