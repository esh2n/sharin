...
| agent-gates | ✅ | 新規執筆。レビューを人の目より前へ寄せる話。指示に書いた関門は経路に無い(2025-07の本番DB削除事故)、生成/統合/出荷の3箇所に置ける関門、コードとテストを同じエージェントが書くと思い込みを共有するのでE2Eの立ち位置が変わること、フラグは止めるのでなく届く範囲を絞ること。採点者を増やすと過剰設計になる罠も一次資料から。実装なし |
