...
| agent-stack | ✅ | 新規執筆。枠組みの5層が実物のどこに当たるかを突き合わせた(Hermes の圧縮 50%/85%・記憶ファイルの字数上限と凍結・delegate_task の真っ新な窓、Pi の道具4つと入れないものの明示)。④に同梱スキルの実態(3つ横並びで優劣なし・推奨は呼び方だけ・自前の子と外部CLIの使い分けは未文書化)、⑤に契約が境目を決める話とサブスク利用者の移行3通りを追加。実装なし・一次資料のみ。追加利用(extra usage)を説明せず引いていた基準①違反を、指摘を受けて修正 |
