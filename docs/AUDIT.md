...
| agent-harness | ✅ | 断定の代わりの「〜になる」5箇所。「失敗すると最初からになる」の言い足らずを補った。**2026-08-02 の指摘**: 5層と言いながら本文が3層ぶんしか無かった(ハーネスは出現5回すべて表か出典で節が無く、題名からも脱落。プロンプトは253字で他節の1/5)。①をプロンプトの層と明示して短い理由まで書き、②を「ハーネスを作り、ループで回す」に改めてハーネスの定義と「単独の節に立たない理由」を追加。題名にハーネスを戻した。**2026-08-01 に全面改訂**: 自認済みの穴だった文脈の層(窓・削る・畳む・窓の外へ書き出す)を実装して測定し、5層と「1回ぶんの単位」の軸、デバッグ指針の節を追加(カバレッジ100%、デモ2本) |
