...
| agent-isolation | ✅ | 新規執筆。回数の上限しか無かった枠組みに範囲の上限を足す視点。確認(実行前・文字列の判断)と隔離(実行後・OS が強制)の違い、包む範囲でBash/プロセス/環境/OS が分かれること、隔離を上げると承認が消える置き換えの関係、鍵と出口とプロンプトインジェクションは境界を越えることを整理。「隔離」が依存の隔離と権限の隔離で二義になる罠も。実装なし・一次資料のみ |
