...
| agent-pipeline | ✅ | 新規執筆。自作グラフの節/辺/Retry/MaxVisits を CI のジョブ・needs・終了コード・--max-turns へ写す対応表と、動く形のワークフロー。決める節を1つに保たないとループに戻ること、やり直しの単位はジョブの切り方で決まること、Cloud Run と Workers が別々に「出す」と「公開する」を分けていること、WIF で鍵を置かない形。②は指摘を受けて実装に測定を追加(訊く節を1→4に増やしても選べることは増えず、費用だけループへ寄る/同じ1つでも後ろの節ほど高く、最初の節は0文字)。カバレッジ100% |
