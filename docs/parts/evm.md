...
- **リバートの意味**: トランザクションは**原子的**であるべきだ。途中で失敗したら、副作用を残さず無かったことにする。DB のトランザクションと同じ。スナップショット + 巻き戻しで実現する。ただし gas は返さない(計算はさせたから)
...
この章の要点は「アカウントモデルは状態を直接持つ。EVM はスタックマシンで、gas が停止性を担保し(命令ごとに価格、尽きたら停止)、失敗時はリバートで状態を巻き戻すが gas は消費する。SSTORE が高いのは永続状態を増やすから」に尽きる。
