...
<Summary>
bytecode 編で作った自前のバイトコード VM を業界標準にしたのが WebAssembly だ。実際の .wasm バイナリをパースし、スタックマシンで実行する最小インタプリタを Go で書く。特徴は制御フローが構造化されていることだ。任意アドレスへ飛ぶ命令はなく、入れ子のブロックとその深さを狙う分岐しかない。飛び先が静的に決まり、実行前に安全性を検証できる。ブラウザが他人のコードを安心して走らせられる理由だ。
</Summary>
