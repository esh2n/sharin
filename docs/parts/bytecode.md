...
この章の要点は「ツリーウォークは木を毎回たどり遅い。だから木を平らなバイトコードにコンパイルし、スタックマシンで回す。式は push/pop、if は条件ジャンプ、変数はスロット番号に還元される。さらに速くするなら computed goto や JIT。JVM/CPython/WASM はこの系統」に尽きる。
