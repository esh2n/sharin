...
先に、言葉を2つ置く。どちらも「言語モデルに仕事をさせる枠組み」の形の名前で、違いは**次に何をするかを誰が決めるか**にある。
...
この章では、その**グラフのほうを実務に置く**。`search → read → edit → test` の順は変わらないので固定し、`edit` の節だけモデルに訊く。落ちたら `edit` へ戻る。同じ節を回りすぎたら止める。

あれをそのまま実務に置くとどうなるか、が この章になる。置き場所はもう手元にある。**CI のワークフローが、すでに節と辺でできている**からだ。ステップが節で、その並びが辺になる。
