...
<Summary>
自分で組んだグラフは節と辺だったが、CI のワークフローも節と辺になっている。写すと対応がそのまま付く。決める節を1つに保てば呼び出しは1回で済み、増やすとループに戻る。そして出す操作と公開する操作は、Cloud Run でも Workers でも別のコマンドに分かれている。フラグの仕組みを別に持たなくても、出荷の関門はそこにある。
</Summary>
...
[エージェントの枠組み](/parts/agent-harness)の⑤で、経路を先に描いてコードで固定する形を作った。`search → read → edit → test` の順は変わらないので固定し、`edit` の節だけモデルに訊く。落ちたら `edit` へ戻る。同じ節を回りすぎたら止める。

あれをそのまま実務に置くとどうなるか、が この章になる。置き場所はもう手元にある。**CI のワークフローが、すでに節と辺でできている**からだ。
...
2. **決める節は1つに保つ**: 各ステップでモデルに訊くと、グラフではなくループになる
...
上のワークフローで、モデルを呼んでいるステップは1つだけだ。ここを崩すと、グラフの利点が消える。

やりがちなのは、各ステップにモデルを置くことになる。「lint の結果を見て判断させる」「テストの失敗をモデルに読ませてから次を決めさせる」。1つずつは自然に見えるが、積むと**毎手訊く形**、つまりループに戻る。枠組みの章の⑤で測ったのがちょうどこの差だった。
...
見分け方は簡単で、**そのステップの次に来るものが、結果によって変わるか**を見る。変わらないなら訊く必要が無い。`npm ci` の次が `npm run lint` なのは決まっている。`npm test` が落ちたときに `edit` へ戻るのも決まっている。決まっていないのは、**どう直すか**だけになる。

判断が要るように見えて実は要らない、というのもよくある。「lint が落ちたらモデルに直させる」は、`npm run lint --fix` で済むことがある。**決定的にできるものを決定的にする**のが、この層でいちばん効く。
...
これは差分だけを新しい文脈で見る形で、[人が見る前に落とす](/parts/agent-gates)の④に当たる。**同じ実行の中で書いた本人に見せるのではなく、PR ができてから別の起動で見る**ところに意味がある。
