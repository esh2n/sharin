...
面接では「全文検索は語→文書リストの転置インデックスが土台。ブール検索はポスティングリストの積と和で、昇順マージで線形に走る。順位づけは TF×IDF が基本で、BM25 が TF 飽和と文書長正規化を足した標準。更新はセグメント + マージで吸収する」を押さえられるかが要。
