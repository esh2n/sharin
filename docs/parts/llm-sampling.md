...
| 方式 | 何で切るか | 特徴 |
|---|---|---|
| greedy | 切らない(argmax) | 決定的。同じ入力から常に同じ出力 |
| temperature | 切らない(形を変える) | 尖り/多様性のトレードオフの主役 |
| top-k | 順位(固定k個) | 単純。分布の形に適応しない |
| top-p | 累積確率 | 分布の形に適応する。長年の実務標準 |
| min-p | 最大確率との比 | 実装が単純で適応的。近年の推論エンジンで採用増 |
