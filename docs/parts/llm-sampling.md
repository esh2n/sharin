...
各節にはその場で動かせるデモを置いてある。どれも Go 版と同じ手順を JS に移しただけのもので、
ブラウザの中だけで計算している。

先に押さえることが3つある。

- LLM の「生成」とは、**logits を確率分布に変えて1トークン抽選する行為の繰り返し**である
- temperature は softmax 直前の**割り算1つ**。特別なパラメータではない
...
- **トークン**: LLM が文章を扱う最小単位。単語より少し細かい(「食べました」が
  「食べ」「まし」「た」のように割れる)。この割り方を決める仕組みは[前章](./bpe)で作った
...
モデルが1ステップで出すのは、語彙の全トークンに対する **logits**(実数の点数の列)だけ。
...
やっていることは割り算1つだが、softmax が exp(指数関数)を通すため効果は大きく、
...
時計を外から渡した[レートリミッタ](./rate-limiter)と同じで、**非決定的なものを注入してテスト可能にする**定石になる。
