...
この点数表から1個選んでコンテキストに足し、また点数表を出す、を繰り返しているだけ。
...
- **モデルと選び方を分ける**: 同じ logits でも、選び方を変えれば出てくる文章の性格が変わる。**モデルを再学習せずに振る舞いを変えられる層**が、ここに1枚あることになる
- **フィルタを1つの形にそろえる**: top-k も top-p も min-p も「切って正規化し直す」に還元できる。同じ形に落とすと、組み合わせても壊れないし、増やすのも簡単になる
- **切る基準が適応するかどうか**: 個数で切ると分布の形に追随できない。累積確率や最大値との比で切ると、尖った場面では狭く、平らな場面では広く、自動で変わる
- **数値の安定化は仕様の一部**: 最大値を引く一手を省くと、大きな logits で全部が無限大になる。**理屈が正しくても計算が壊れる**ところを、実装で塞いでおく
- **決定的にできる口を残す**: 温度 0 と乱数の注入で、同じ入力から同じ出力を出せる。再現性が要る用途と、多様性が要る用途を同じコードで賄える
- **順番に意味がある**: 温度で形を作ってから切るのか、切ってから温度をかけるのかで結果が変わる。パイプラインの並びそのものが設計になっている
...
- **nucleus sampling の動機**: top-p の原論文は、greedy や beam search が繰り返しに陥る現象を観察するところから始まっている。**尤度を最大にすると、人間の書く文章から遠ざかる**という逆説がこの分野の出発点になった
- **温度 0 でも完全な再現にはならない**: 同じ入力でも、バッチの組み方や GPU の並列加算の順序で浮動小数の丸めが変わり、argmax が入れ替わることがある。API が決定性を保証しないのはこのため
- **繰り返しへの対処は別のつまみ**: 同じ語が続くのを抑える repetition penalty や frequency penalty は、過去に出したトークンの logit を下げる処理になる。この章のフィルタと同じ層に、履歴を見る係が並ぶ
- **min-p は新しい**: 提案が 2024 年と比較的新しく、実装が単純なわりに分布の尖りへ適応する。llama.cpp や vLLM といったローカル・自前運用の推論エンジンから広まった
- **既定値は用途で違う**: コード生成は温度を低く、創作は高く。API の既定が 1.0 前後なのは中庸を取っているだけで、**用途ごとに調整する前提の値**になっている
