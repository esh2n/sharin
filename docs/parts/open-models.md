...
[GPT系譜](/parts/gpt-lineage) で見たとおり、フロンティアの GPT-4 以降は構成すら非公開になった。その空白を埋めるように伸びたのがオープンモデルだ。重みが公開され、自分のマシンで動かせて、改造もファインチューニングもできる。そして技術レポートが読めるので、[Transformerの部品](/parts/transformer)で 1 つずつ実装してきた RoPE・GQA・RMSNorm・SwiGLU・MoE が、実物でどう組み合わされているかを確認できる。この章は部品編の答え合わせでもある。
