...
面接では「ロールアップは実行を L2 に逃がし L1 は root を記録するだけ。正しさの担保が optimistic(fraud proof + challenge 期間 + slashing、安いが遅く監視者前提)と zk(validity proof、即確定・監視者不要だが証明が重い)に分かれる。どちらも data availability が前提」を押さえられるかが要。
