...
**効き方が面白い**。使えなくなるのではなく、**どの財布から出るかが変わる**。Hermes と Pi が、それぞれ独立に同じことを書いている。

| | 書いてあること |
|---|---|
| Pi | Anthropic のサブスク認証は Claude Pro/Max で有効。ただし第三者ハーネスからの利用は extra usage から引かれ、トークン単位で課金される。**Claude のプラン上限に対しては数えられない** |
