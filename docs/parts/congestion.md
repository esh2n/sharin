...
<Summary>
帯域は共有資源だ。送り手が一斉に全力で送れば経路が詰まり、パケットが溢れ、再送でさらに詰まる。だが誰も空き帯域を知らない。TCPは各接続が自分で探る。輻輳ウィンドウをスロースタートで倍々に増やして空きを探り、閾値からは1往復に1ずつ慎重に増やす。落ちたら詰まった合図とみなし半分に切る。この制御を実装し、線形に増やし詰まったら半減するAIMDが、中央の調整役なしに効率と公平を両立する仕組みを見る。
</Summary>
...
<FigureBox caption="cwnd の推移。スロースタートで倍々に増え、ssthresh から加算増加に切り替わる。損失で半減し、また増やす。容量を探るのこぎり波">
...
- **AIMD が公平の鍵**: 加算増加・乗算減少の組み合わせが、公平点への収束を生む。加算増加・加算減少や乗算増加では収束しない。この非対称が本質
