...
この章もコードを書かない。[エージェントの枠組み](/parts/agent-harness)で作った枠組みには、止め方が1種類しか入っていなかった。**回数の上限**だ。ループには手数、グラフには同じ節への訪問回数を置いた。[Hermes と Pi で読み替える](/parts/agent-stack)で見た実物も同じで、反復50、`--max-turns`、並列3、どれも回数を数えている。
