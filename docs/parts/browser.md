...
<Summary>
HTML文字列と CSS文字列から「画面のどこに何を描くか」を計算する——ブラウザのレンダリングエンジンの背骨を最小構成で作る。フロント編の締めくくり。肝は3つ。<strong>段階に分ける</strong>——parse → style → layout → paint の一方通行。各段は前段の出力だけを入力に取るので、単体で理解・テストできる。<strong>スタイルはカスケード + 継承</strong>——同じプロパティが複数当たれば詳細度(id>class>tag)の高い規則が勝ち、color などは子へ受け継ぐ。<strong>レイアウトは箱の入れ子</strong>——ブロックは親幅を満たし、子は上から下へ積む。padding は内側、margin は外側。<a href="./mini-next">SSR</a> の「木→HTML文字列」の逆から始まり、最後は箱を実際に描く。
</Summary>

## この章で作るもの

[mini-next](./mini-next) の SSR は「仮想DOM木 → HTML文字列」だった。ブラウザはその**逆から始める**。受け取った HTML文字列を DOM 木に戻し、CSS を当て、各要素の位置と大きさを計算し、最後に「ここに この色の矩形」「ここに この文字」という描画コマンドにする。URL を入れてから画面が出るまでの、レンダリング部分の核を取り出す。
...
「幅は親から下りてきて、高さは子から上がってくる」——この向きの違いがブロックレイアウトの勘所。
...
**親を先に、子を後に**積むのが要。後のコマンドが手前に描かれるので、親の背景の上に子が正しく重なる。

### 動かす

下のデモは、`frontend/browser` の `render(html, css, width)` をそのまま呼んでいる。左に入力の HTML/CSS と、パイプラインが吐いた**描画コマンド一覧**。右は、そのコマンドを**そのまま矩形と文字として描いた結果**——座標も色もパイプラインの計算値をそのまま使っている。サンプルを切り替えて、CSS の padding / margin / height がコマンドの座標にどう効くかを見てほしい。

<BrowserDemo />

これで、HTML と CSS の文字列から実際に「箱の絵」が出るところまで繋がった。ここまでがブラウザのレンダリングの背骨。

## アーキテクチャ面接の観点: なぜ段に分け、なぜ再レイアウトが重いのか
...
- **レイアウトスラッシング**: JS で「スタイルを変える → 位置を読む → また変える」を繰り返すと、読むたびに強制同期レイアウトが走って劇的に遅くなる。読みと書きをまとめろ、の理由がこれ
- **合成(compositing)**: `transform` や `opacity` のアニメは layout/paint を飛ばして合成だけで動かせる(GPU)。だから「位置を動かすなら `left` より `transform`」と言われる

面接では「パイプラインは段で分かれ、変更がどの段から波及するかでコストが決まる。layout を触る変更が最も高い」を押さえられるかが要。
...
- **React / Vue の「仮想DOM」との関係**: あれは[このパイプラインの手前](./vdom)——「DOM をどう変えるか」を決める層。DOM が変われば、その先でブラウザがこのパイプラインを回す
