...
<Summary>
OS のいちばん奥にある仕事、「誰が CPU を握るかを決める」を Go でモデル化する。CPU は 1 つ、走れるタスクは同時に 1 つ。それを何本ものタスクで分け合うのがスケジューラだ。ここで作るのは協調(cooperative)方式で、タスクが自発的に yield / sleep して CPU を手放したときにだけ次を選ぶ。横取りはしない。タスクの文脈は pc に尽き、context switch とはその位置の保存と復元にすぎない。yield を書かない貪欲なタスクは他を丸ごと待たせる。
</Summary>
