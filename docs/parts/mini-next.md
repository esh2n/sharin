...
下のデモで、URL を選ぶとルータがページを解決し、SSR が HTML文字列を作り、ブラウザが**静的に**描画する。この時点ではまだ**未ハイドレート**で、「いいね」ボタンを押しても動かない(イベント未接続)。「ハイドレート」を押すと、既存DOMはそのままにイベントだけが付き、ボタンが反応するようになる。SSR とハイドレーションの間にある「見えるが動かない」瞬間を体感してほしい。
...
この章の要点は「SSR は初回表示と SEO のため。だがハイドレーションという二度手間が付きまとい、そこを削るのが最近の潮流(ストリーミング・部分ハイドレーション・サーバコンポーネント)」に尽きる。
