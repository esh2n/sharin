...
Listener がその名前空間を受け入れていること。これは運用側の意思で、勝手に相乗りされない保証になる。既定が「同じ名前空間だけ」になっているのが大事なところで、開くには明示が要る。閉じているほうを既定にするこの形は、後の [NetworkPolicy](/parts/network-policy) や [RBAC](/parts/rbac) の章にも繰り返し出てくる。
