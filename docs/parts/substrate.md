...
<Summary>
多くのチェーンは、送金や合意の規則をノードのソフトに焼き込む。規則を変えるには全ノードが更新してハードフォークするしかない。Substrate はランタイム(規則)をチェーン上の差し替え可能なコードとして持ち、機能追加や変更を取引 1 本で行う。ノードもフォークも要らない(forkless upgrade)。ランタイムは pallet の合成でできた状態遷移関数で、extrinsic を dispatch し、1 ブロックの仕事量は weight で予算化する。
</Summary>
...
面接では「Substrate はランタイムをチェーン上の差し替え可能な Wasm として持ち、set_code 取引でフォークせず更新する。ランタイムは pallet の合成で、extrinsic を dispatch して状態を進め、weight で 1 ブロックの仕事量を予算化する。状態はアップグレードをまたいで保たれ(必要なら migration)、差し替え権限はガバナンスで絞る」を押さえられるかが要。
...
- **state migration なし**: ストレージの形を変えるアップグレードと、その移行処理は扱わない(面接節で言及)
