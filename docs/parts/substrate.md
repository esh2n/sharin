...
この章の要点は「Substrate はランタイムをチェーン上の差し替え可能な Wasm として持ち、set_code 取引でフォークせず更新する。ランタイムは pallet の合成で、extrinsic を dispatch して状態を進め、weight で 1 ブロックの仕事量を予算化する。状態はアップグレードをまたいで保たれ(必要なら migration)、差し替え権限はガバナンスで絞る」に尽きる。
