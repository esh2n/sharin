...
<Summary>
「A が B に送金する」と聞くと、A の残高から引いて B の残高に足す——銀行の台帳のような操作を思い浮かべる。ところが Bitcoin をはじめとする多くのチェーンは、<strong>残高という数字をどこにも持たない</strong>。あるのは「まだ誰にも使われていない出力(<strong>UTXO</strong> = Unspent Transaction Output)」の集まりだけだ。取引は過去の出力を <strong>input</strong> で指して<strong>消費</strong>し、新しい <strong>output</strong> を生む。財布の中の硬貨を出して、お釣りを受け取るのと同じ——1000 円の品を 5000 円札で買えば、5000 円札は無くなり、品物と 4000 円のお釣りが手元に来る。残高は「自分あての未使用出力を数え上げて」初めて分かる。この仕組みが、中央の台帳なしに 2 つのことを同時に成立させる: <strong>所有権</strong>——出力には受取人の公開鍵が刻まれ、消費にはその鍵の署名が要る。<strong>二重支払いの防止</strong>——一度消費された出力は集合から消え、同じものを二度は使えない。「送金 = 消費と生成」「残高 = 集計」「正しさ = 署名 + 一意消費」。この 3 つが UTXO モデルの全てだ。
</Summary>

## この章で作るもの

[blockchain 編](/parts/blockchain)では、ハッシュチェーンと Proof of Work で「改竄しにくい追記ログ」を作った。だがそこには肝心の**取引**が無かった——ブロックの中身はただの文字列だった。この章はその中身、「A が B に送る」の正体を作る。

鍵になる問いは 2 つ。**中央の銀行なしに、どうやって「誰が何を持っているか」を決めるのか**。そして、**どうやって「他人の金を勝手に使えない」ようにするのか**。UTXO モデルの答えは、残高を持たないことだった。
...
取引は「過去の出力を消費し、新しい出力を生む」ものだ。だから 2 つの部品でできている。**output** は「誰にいくら」——受取人の公開鍵(Owner)と金額を刻む。**input** は過去の出力を 1 つ指す参照(`OutPoint`)と、それを消費してよいことを示す**署名**を持つ:
...
`Apply` の `delete` が二重支払い防止の核心だ。一度消費された `OutPoint` は集合から消える。だから後から同じ出力を指す取引が来ても、「そんな UTXO は無い」と弾かれる。**二重支払いは、集合から消えることそのもので防がれる**——特別なフラグも履歴走査も要らない。
...
面接では「UTXO は残高を持たず、未使用出力の集合で表す。送金は input 消費 + output 生成で、二重支払いは一意消費で防ぐ。所有権は出力側の公開鍵への署名で守る。状態を持ちたいならアカウントモデル」を押さえられるかが要。
