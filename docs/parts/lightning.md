...
<Summary>
ブロックチェーンは 1 秒に数件しかさばけない。コーヒー 1 杯を毎回チェーンに載せるのは遅く高い。Lightning は、2 者が一度だけ資金をロックし、以降の残高更新を署名済みの割り当ての差し替えとしてオフチェーンで行う。チェーンに触れるのは開閉の 2 回だけだ。ただし古い残高を提出して過去に巻き戻す不正が生まれる。これをリボケーションと全額没収のペナルティで封じ、直接繋がらない相手へは HTLC を多段に張って届ける。
</Summary>
...
面接では「Lightning は funding と close だけ L1 を使い、間の送金は署名済み commitment の差し替えでオフチェーンに出す。古い状態の提出はリボケーションによる全額没収で抑止する。直接繋がらない相手へは preimage のハッシュで縛った HTLC を多段に張り、preimage の伝播で経路を原子的に成立させる」を押さえられるかが要。
