...
<Summary>
もう要らないメモリを、誰がどうやって見つけて返すのかを Go でモデル化する。判定基準は到達可能性で、スタックやグローバル(ルート)から辿れるものだけが生き、辿れないものは回収してよい。実装の肝は tricolor マーキングで、white / gray / black の 3 色で到達集合を広げ、残った white を掃く。参照カウントが漏らす循環参照も、互いを指し合っていてもルートから辿れなければゴミなので回収できる。
</Summary>
