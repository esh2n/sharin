...
<Summary>
もう要らないメモリを、誰がどうやって見つけて返すのかを Go でモデル化する。GC が答える問いはただ 1 つ、このオブジェクトはもう要らないか。判定基準は到達可能性だ。スタックやグローバル(ルート)から辿れるものだけが生きており、辿れないものは回収してよい。実装の肝は tricolor マーキングで、white / gray / black の 3 色で到達集合を広げ、残った white を掃く。参照カウントが漏らす循環参照も、互いを指し合っていてもルートから辿れなければゴミなので回収できる。
</Summary>
