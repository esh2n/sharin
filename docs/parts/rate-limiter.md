...
<Summary>
サーバーに来るリクエストの数を、単位時間あたりで頭打ちにする仕組みを作る。token bucket から sliding window まで4方式を書いてみると、どれも「一瞬に集中するアクセス(バースト)をどこまで許すか」の選び方が違うだけだと分かる。何を守りたいかで正解が変わる。固定窓は窓の変わり目で上限の2倍を通してしまう、といった穴も数えて確かめる。
</Summary>
...
- **何を守りたいのかで方式が決まる**: 平均レートを守りたいのか、下流の瞬間的な処理能力を守りたいのか、期間ごとの総量を守りたいのか。**同じ「制限する」でも、守る対象が違えば正解が違う**
- **持つ状態の量が、そのまま正確さの値段**: 数値2つで足りるものから、時刻を全部覚えるものまで幅がある。厳密さが欲しければ記憶を払う
- **時刻を外から渡す**: 経過時間で判断する仕組みは、時計を注入できる形にしないとテストが書けない。**非決定的なものを引数にする**のが、この種の部品の定石になる
- **バーストを許すかどうかは体験の設計**: 貯めた分を一気に使えると、たまにしか来ない利用者に優しい。均すと下流は守れるが、待たされる感覚が出る
- **境界の弱点は算数で出る**: 固定窓は窓の変わり目に上限の2倍を通せる。この種の穴は運用で気づくのでなく、設計時に数えて分かる
- **どこに状態を置くかは別の問題**: アルゴリズムは1台でも複数台でも同じで、変わるのは残量をどこに置き、どうやって同時更新を防ぐかだけになる
...
- **固定窓の2倍問題**: 窓の終わり際に上限ぶん、次の窓の頭にもう一度上限ぶんを通すと、1窓ぶんの時間に2倍が流れる。**上限を守っているのに守れていない**という、期間の切り方から出てくる穴になる
- **sliding window counter という折衷**: 前の窓のカウントを、はみ出した割合ぶんだけ按分して足す近似がある。時刻を全部覚えずに境界の穴をほぼ塞げるので、実務ではこれが有力になる
- **返し方まで含めて仕様**: 拒否するときは 429 を返し、`Retry-After` でいつ再試行してよいかを伝える。残量を `RateLimit` 系のヘッダで返す取り決めも標準化が進んでいる。**制限は伝えて初めて相手が正しく振る舞える**
- **クライアント側の作法と対になる**: 制限された側が即座に再試行すると、混雑をさらに悪くする。[リトライとバックオフ](./retry-backoff)で見た指数バックオフとジッタが、制限する側の設計と対になっている
- **複数台では原子性が主題になる**: 残量を Redis に置き、読み取り・計算・書き込みを Lua スクリプトで1つにまとめるのが定石。アルゴリズムは変わらず、変わるのは同時更新をどう防ぐかだけになる
