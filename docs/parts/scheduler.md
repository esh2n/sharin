...
<Summary>
Go がどうやって数百万の goroutine を数個の OS スレッドで回すのか——その心臓部、<strong>M:N スケジューラ(GMP モデル)と work-stealing による負荷分散</strong>を Go でモデル化する。<a href="/parts/os">os 編</a>の協調スケジューラは CPU 1 個・実行キュー 1 本だった。実機はコアが複数ある。キューを 1 本にすればロックがボトルネックになり、コアごとに独立させれば偏ったとき暇なコアが遊ぶ。Go の答えが GMP + work-stealing だ——<strong>G</strong>(goroutine)、<strong>M</strong>(OS スレッド)、<strong>P</strong>(プロセッサ。ローカル実行キューを持つ)。M は P を握らないと G を走らせられず、P の数が並列度を決める。仕事は基本、生成した P のローカルキューに積む(ロック無しで速い)が、偏る。そこで<strong>ローカルキューが空になった P は、他の最も混んでいる P からキューの半分を横取りする(work-stealing)</strong>。これで偏りが自動的に均され、全コアが遊ばない。あふれたぶんは<strong>グローバルキュー</strong>へ退避し、そこからも配る。os 編の「1 本のキュー」が「P ごとのキュー + 盗み合い」に進化した姿だ。
</Summary>
...
まず登場人物を定義する。**G** は goroutine——走らせたい仕事(ここでは `work` tick を消費し切ったら終わる)。**P** はプロセッサ——スケジューリングの文脈で、自分だけのローカル実行キューを持つ。M(OS スレッド)は「P 1 つにつき常に 1 つある」と割り切って省く(本章の主題は P 間の仕事の均し方だから):
...
`Scheduler` は複数の P と、全 P が共有するグローバルキューを持つ。`Go` は goroutine を生成して P0 のローカルキューに積む——「1 つの goroutine から生まれた仕事は同じ P に溜まる」を模す。ローカルが上限に達したら、半分をグローバルへ退避する(spill):
...
`steal` が均衡の要だ。盗むのが**半分**なのは、盗みすぎず・盗まなさすぎずの勘所——全部盗むと今度は相手が空になって盗み返しが起き、少ししか盗まないと何度も盗みに行く羽目になる。半分なら、盗んだ側も盗まれた側も次の仕事を持てる。`quantum` を使い切った G をローカル末尾へ戻すのは、os 編の yield に当たる協調的な切り替え点だ。
...
## アーキテクチャ面接の観点: なぜ M:N + work-stealing か
...
面接では「goroutine は M:N で少数スレッドに多重化する。P ごとのローカルキューでロック競合を避け、偏りは work-stealing(暇な P が半分盗む)で均す。グローバルキューであふれと飢餓を扱い、非同期プリエンプションで占有を防ぐ」を押さえられるかが要。
...
- **グローバルの定期チェック省略**: 実機の「約 61 回に 1 回グローバル優先」は入れない(仕組みは面接節で説明)
