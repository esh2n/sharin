...
<Summary>
Go がどうやって数百万の goroutine を数個の OS スレッドで回すのか、その心臓部である M:N スケジューラ(GMP モデル)と work-stealing を Go でモデル化する。キューを 1 本にすればロックがボトルネックになり、コアごとに独立させれば偏ったとき暇なコアが遊ぶ。Go の答えは、P ごとにローカルキューを持たせてロック無しで回し、空になった P が最も混む P からキューの半分を横取りすることだ。偏りが自動的に均され、全コアが遊ばない。
</Summary>
