...
<Summary>
Express も Rails も Go の net/http も、下でやっていることは同じ。TCP で届いた「ただのテキスト」を読んで、それに応じたテキストを返しているだけ。この章では net.Listen("tcp") で生のバイト列を受け取り、HTTP リクエストを手でパースして、レスポンスを手で組む。フレームワークが隠しているものの正体が、200行ほどで丸見えになる。
</Summary>
...
この章は TCP そのものは Go の `net` に任せ(自作 TCP は発展)、HTTP の層を作る。
...
1接続の処理はこれだけ——生のバイト列からパースし、ルートを引いて、書き戻す:
...
`curl http://localhost:...` でも叩ける本物のサーバ。

## メリット / デメリット(この自作実装の)

**メリット**

- HTTP の全体像が見える。フレームワークの「魔法」がなくなる
...
- **HTTP/2**: 1つの TCP 接続で複数リクエストを多重化(binary framing)。テキストではなくバイナリ
- **HTTP/3**: TCP をやめて UDP ベースの QUIC に載せ替え、接続確立を速くした
- **TLS**: 全部を暗号化する層。crypto 編でハンドシェイクの中身を作る予定
...
- **TCP は net 任せ**: 3-way handshake や再送は Go が担う。自作 TCP は発展テーマ
