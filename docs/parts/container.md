...
<Summary>
「コンテナ」の正体を Go でモデル化する。Docker が魔法に見えても、中身は Linux カーネルの 2 つの機能の組み合わせにすぎない。namespace は、PID・ポート・マウント・ホスト名という本来 1 つしかない資源を、コンテナごとに別インスタンスに見せる。だから各コンテナの init がどちらも PID 1 に見え、2 つが同時に :80 を掴める。cgroup は、メモリやプロセス数の上限を木構造の予算として持ち、超えたら全か無かで拒否する。コンテナは VM ではなく、1 つのカーネルを共有したまま制限を与えたものだ。
</Summary>
