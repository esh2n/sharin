...
分けているのは、順序を保証するためになる。書き換えが全部終わってから、検証が始まる。だから書き換えで足したものも検証の対象になり、書き換えで壊したものは検証で止まる。もし1つの関門が書き換えと検証を両方やって、それが順不同に並ぶと、検証を通った後で書き換えられたものが素通りしてしまう。誰も見ていないものが保存される。
