...
[学習パイプライン](/parts/llm-training) の SFT や RLHF は、モデルの全パラメータを更新するフル微調整だった。だが 70B モデルをフル微調整するには、重みだけでなく勾配とオプティマイザの状態(Adam なら重みの 2 倍)も持つ必要があり、必要メモリは推論の数倍に膨れる。手元の GPU で公開モデルを自分用に調整したくても、この壁に阻まれる。
