...
[GC](/parts/gc)で、使われなくなったメモリを自動で回収する仕組みを見た。だがその一段下、そもそも「メモリを確保する」とはどういうことか。`malloc(n)` や、Go の `make`、`new` を呼ぶと n バイトが返ってくる。この裏で、アロケータが働いている。OS から受け取った 1 枚の連続したメモリ(ヒープ)を管理し、プログラムの要求に応じて必要な分を切り出して貸し、返されたら次に備えて回収する。銀行の窓口が、大きな金庫から必要な額を出し入れするようなものだ。
