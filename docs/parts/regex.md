...
面接では「正規表現エンジンはバックトラッキング型とオートマトン型に分かれる。オートマトン型は Thompson 構成で NFA にし、状態集合を並行に進めるのでバックトラッキングが要らず線形時間。DFA に変えれば 1 文字 1 表引きだが状態数が指数的になりうる。後方参照が要るならバックトラッキング型を選ぶ」を押さえられるかが要。
