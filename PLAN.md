...
| パーツ | 状態 | 備考 |
|---|---|---|
| tcp | ✅ | 信頼バイトストリーム。3-wayハンドシェイク・seq/累積ACK・再送・スライディングウィンドウ・並べ替え。損失注入で決定的。96.5%cov・-race |
...
| scheduler | ✅ | M:N スケジューラ(GMP)+work-stealing。P毎ローカルキュー・暇なPが忙しいPから半分横取り・グローバルキューへspill。負荷が均衡。100%cov・-race |
| wasm | ✅ | 最小WebAssemblyインタプリタ。実.wasmバイナリをLEB128パース、i32スタックマシン、構造化制御フロー(block/loop/if+br=脱出/継続)、関数/再帰。87.9%cov・-race |
